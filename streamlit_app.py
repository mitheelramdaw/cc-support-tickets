...
if "logged_in" not in st.session_state or not st.session_state.logged_in:
    ...
    if tickets:
        ...
        # Bar chart for count of tickets by priority
        priority_count = df.groupby("Priority").size().reset_index(name="Count")
        priority_bar_plot = (
            alt.Chart(priority_count)
            .mark_bar()
            .encode(x="Priority:N", y="Count:Q", color="Priority:N")
            .properties(title="Tickets Count by Priority")
        )
        st.altair_chart(priority_bar_plot, use_container_width=True)
